      ...
      <div class="postInfo-container">
        <el-row>
          <el-col :span="10" class="min-width">
            <el-form-item label-width="140px" label="内容方名称：" prop="name">
              <el-input v-model="postForm.name" placeholder="请填写内容方名称"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-col :span="10" class="min-width">
            <el-form-item label-width="140px" label="联系方式：" prop="phone">
              <el-input v-model="postForm.phone" placeholder="请填写联系方式"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="10" class="min-width">
            <el-form-item label-width="140px" label="电子邮件：" prop="email">
              <el-input v-model="postForm.email" placeholder="请填写电子邮件"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="10" class="min-width">
            <el-form-item label-width="140px" label="内容方网站：" prop="home_url">
              <el-input v-model="postForm.home_url" placeholder="请填写内容方网站"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
import { fetchManufacturer, createManufacturer, updateManufacturer } from '@/api/manufacturer'
  ...
  methods: {
    submitForm() {
      ...
      this.$refs.postForm.validate(valid => {
        if (valid) {
          createManufacturer(this.postForm).then(res => {
            // console.log(res)
            if (res.data.code === 0) {
              this.$message({
                type: 'success',
              ...
              this.jumpTo('/manage/manufacturer', true)
            } else {
              this.$message({
                type: 'error',
    ...
    saveForm() {
      ...
      this.$refs.postForm.validate(valid => {
        if (valid) {
          updateManufacturer(this.postForm).then(res => {
            if (res.data.code === 0) {
              this.$message({
                type: 'success',
                message: '保存成功!'
